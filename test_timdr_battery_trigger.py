"""
test_timdr_battery_trigger.py — testy timdr_battery_trigger.py.

Ten sam wzorzec i te same fixtury co w siostrzanym repo
TIMDR-Industrial-Predict/test_timdr_industrial_trigger.py, bo
TIMDRBatteryFusion._mad_z()/twist()/anomalies() to identyczna
implementacja (te same progi 3.5/3.0, ten sam fallback rozstęp/4 przy
MAD=0).

1. test_anomaly_na_realnym_pojedynczym_skoku - JEDEN test integracyjny na
   prawdziwym TIMDRBatteryFusion (bez mockowania).
...
from timdr_battery_trigger import BatteryTrigger, BatteryTriggerType


# ----------------------------------------------------------------------
# 1) Test integracyjny na realnym TIMDRBatteryFusion
# ----------------------------------------------------------------------

def test_anomaly_na_realnym_pojedynczym_skoku():
    """
    E(t) = [10]*10 z pojedynczym skokiem do 20 w idx=5.
    _mad_z: mediana=10, mad_raw=0 (9 z 10 wartosci to 0) -> fallback
    span/4 = 2.5. z[5]=(20-10)/2.5=4.0 > 3.0 (prog anomalies())
    -> anomalies() zwraca [5].

    twist(): druga pochodna dds=[0,0,0,2.5,0,-5,0,2.5,0,0], fallback
    span/4=(2.5-(-5))/4=1.875, z_max=5/1.875=2.667 < prog 3.5
    -> twist() PUSTE, wiec to naprawde ANOMALY, nie STRUCTURE.
    """
    t = list(range(10))
    E = [10.0] * 10
    E[5] = 20.0
    ...
    assert len(twist_idx) == 0  # sanity

    trigger = BatteryTrigger(predictor=_InfPredictor())
    result = trigger.analyze(t, E)

    assert result.triggered is True
    assert result.trigger_type == BatteryTriggerType.ANOMALY
    assert result.location == 5


class _InfPredictor:
    """Stub predict_failure() zawsze zwracajacy 'brak przewidywanej
    awarii' (inf) - uzywany, gdy test chce dojsc do STRUCTURE/ANOMALY/
    NONE bez ingerencji warstwy FAILURE_IMMINENT."""
...
def test_priorytet_failure_imminent_nad_wszystkim():
    ...
    trigger = BatteryTrigger(alert_ttf_seconds=3600.0, fusion=fusion, predictor=predictor)
    t, E = _dummy_args()
    result = trigger.analyze(t, E)
    assert result.trigger_type == BatteryTriggerType.FAILURE_IMMINENT
...
def test_ttf_powyzej_progu_nie_odpala_failure_imminent():
    ...
    t, E = _dummy_args()
    result = trigger.analyze(t, E)
    assert result.trigger_type == BatteryTriggerType.STRUCTURE
    assert result.location == 2


def test_priorytet_structure_nad_anomaly():
    fusion = _FakeFusion(twist_idx=[3], anomaly_idx=[0])
    trigger = BatteryTrigger(fusion=fusion, predictor=_InfPredictor())
    t, E = _dummy_args()
    result = trigger.analyze(t, E)
    assert result.trigger_type == BatteryTriggerType.STRUCTURE
...
def test_none_gdy_wszystko_puste():
    ...
    assert result.trigger_type == BatteryTriggerType.NONE
